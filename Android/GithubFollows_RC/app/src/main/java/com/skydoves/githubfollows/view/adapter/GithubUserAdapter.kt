...
class GithubUserAdapter(val delegate_header: GithubUserHeaderViewHolder.Delegate,
    ...
    private val section_user_github_repositories=2
    ...
    fun addUserGitHubRepositories(repositories:List<UserGitHubRepositories>){
        sections[section_user_github_repositories].addAll(repositories)
        notifyDataSetChanged()
    }
    fun clearAll() {
        sections[section_header].clear()

...
public class DataSet {
	...
	public double getInformationFor() {
		...
		//System.out.println("Information:"+Util.information(data));
		return Util.information(data);
	}
	
	public double getInformationFor(String attrib) {
		//String attributeName = specification.get();
		Hashtable<String, Integer> counts = new Hashtable<String, Integer>();
		for (Example e : examples) {

			String val = e.getAttributeValueAsString(attrib);
			if (counts.containsKey(val)) {
				counts.put(val, counts.get(val) + 1);
			} else {
				counts.put(val, 1);
	...
	//
	/******************
	 * function to calculte the Split info....
	 * @param attributeName
	 * @return
	 ***********/
	public double getSplitInfo(String attributeName){
		double splitInfo=0.0;
		//System.out.println("getting splitinfo for:"+attributeName);
		Hashtable<String, DataSet> hash = splitByAttribute(attributeName);
		double totalSize = examples.size();
		//System.out.println("total Size:"+totalSize);
		for(String val:hash.keySet()){
			double reducedDataSetSize = hash.get(val).examples.size();
			//System.out.println("Size of "+val+": "+reducedDataSetSize);
			splitInfo=splitInfo+(Util.log2(reducedDataSetSize/totalSize)*(-1));
		}
		return splitInfo;
	}

...
public static void decisionTreeDemo(){
		try {
			...
			for (int i=0;i<partitions.size();i++){
				DataSet learnData = new DataSet(createDiabetesDataSetSpec());
			...
			System.out.println("\nThis Decision Tree classifies the data set with Gain "+ results[0]+ " successes"+ " and "	+ results[1]+ " failures");
			...
			System.out.println("\nThis Decision Tree classifies (with Gain Ratio)the data set with "
...
public static DataSet getDiabetesDataSet() throws Exception {
	...
	return  f.fromFile("diabetes", spec, ",");
}
...
public static DataSet getDiabetesDataSetDisc() throws Exception {
	DataSetSpecification spec = createDiabetesDataSetSpec();
	return new DataSetFactory().fromFile("diabetes", spec, ",");
}
}
